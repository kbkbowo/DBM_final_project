from django.urls import path
from . import views

urlpatterns = [
    path('login/', views.login, name='login'),
    path('', views.home, name='home'),
    path('logout/', views.logout, name='logout'),
    path('profile/', views.profile, name='profile'),
    path('edit_profile/', views.edit_profile, name='edit_profile'),
    path('signup/', views.signup, name='signup'),
    path('manage_users/', views.manage_users, name='manage_users'),
    path('manage_orgs/', views.manage_orgs, name='manage_orgs'),
    path('manage_hospitals/', views.manage_hospital, name='manage_hospital'),
    path('manage_hospitals/add/', views.hospital_add, name='hospital_add'),
    path('manage_hospitals/<str:hospital_id>/delete/', views.hospital_delete, name='hospital_delete'),
    path('manage_hospitals/<str:hospital_id>/edit/', views.hospital_edit, name='hospital_edit'),
    path('org_home/', views.org_home, name='org_home'),
    path('org_build/', views.org_build, name='org_build'),
    path('org_browse/', views.org_browse, name='org_browse'),
    path('org_home/<str:org_id>/', views.org_page, name='org_page'),
    path('org_home/<str:org_id>/edit_info/', views.org_edit_info, name='org_edit_info'),
    path('org_home/<str:org_id>/edit_founder/', views.org_edit_founder, name='org_edit_founder'),
    path('org_home/<str:org_id>/delete/', views.org_delete, name='org_delete'),
    path('org_home/<str:org_id>/leave/', views.org_leave, name='org_leave'),
    path('org_home/<str:org_id>/join/', views.org_join, name='org_join'),
    path('org_home/<str:org_id>/event_panel/', views.org_event_panel, name='org_event_panel'),
    path('org_home/<str:org_id>/event_panel/create_event/', views.org_create_event, name='org_create_event'),
    path('org_home/<str:org_id>/event_panel/<str:event_id>/delete_event/', views.org_delete_event, name='org_delete_event'),
    path('org_home/<str:org_id>/animal_panel/', views.org_animal_panel, name='org_animal_panel'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/shelter/', views.org_shelter_animal, name='org_shelter_animal'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/release/', views.org_release_animal, name='org_release_animal'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/adopt/', views.org_adopt_animal, name='org_adopt_animal'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/adopt/<str:adopt_user_id>/', views.org_adopt_animal_confirm, name='org_adopt_animal_confirm'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/hospital/', views.org_hospital_animal, name='org_hospital_animal'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/hospital/<str:hospital_id>/send/', views.org_send_animal, name='org_send_animal'),
    path('org_home/<str:org_id>/animal_panel/<str:animal_id>/hospital/<str:hospital_id>/bring_back/<str:sent_date>/', views.org_take_back_animal, name='org_take_back_animal'),
    path('org_home/<str:org_id>/visit_panel/', views.org_visit_panel, name='org_visit_panel'),
    path('org_home/<str:org_id>/visit_panel/<int:visit_id>/approve/', views.org_visit_approve, name='org_visit_approve'),
    path('org_home/<str:org_id>/visit_panel/<int:visit_id>/reject/', views.org_visit_reject, name='org_visit_reject'),
    path('org_home/<str:org_id>/donation_panel/', views.org_donation_panel, name='org_donation_panel'),
    path('event/', views.event, name='event'),
    path('event_browse/', views.event_browse, name='event_browse'),
    path('event/<str:event_id>/join/', views.event_join, name='event_join'),
    path('event/<str:event_id>/quit/', views.event_quit, name='event_quit'),
    path('report_animal/', views.report_animal, name='report_animal'),
    path('adopt_animal/', views.adopt_animal, name='adopt_animal'),
    path('adopt_animal/<str:animal_id>', views.adopt_animal, name='adopt_animal'),
    path('my_schedule/', views.my_schedule, name='my_schedule'),
]
